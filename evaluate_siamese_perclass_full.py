...
def load_model(model_module: str, embedding_dim: int, pretrained: bool, device: str):
    ...
    model_module example:
      "src.models.mobilenetv3_plus_siamese_exact"
      "src.models.resnet18_siamese"
      "src.models.vgg19_siamese"
    The module must expose SiameseNet and pairwise_l2.
    """
    m = importlib.import_module(model_module)
    SiameseNet = getattr(m, "SiameseNet")
    pairwise_l2 = getattr(m, "pairwise_l2")
    model = SiameseNet(embedding_dim=embedding_dim, pretrained=pretrained).to(device)
...
def forward_pair_embeddings(model, x1, x2):
    ...
    Supports:
      - model(x1, x2) -> (z1, z2)
      - model.forward_pair(x1, x2) -> (z1, z2)
      - model.forward_once() fallback
    """
...
def main(args):
    ...
    save_confusion_heatmap(
        ...
        title="False-Accept Confusion Matrix (Pred SAME on Different Writers)",
    ...
    save_confusion_heatmap(
        ...
        title="False-Reject Matrix (Pred DIFF on Same Writer) — Diagonal shows FN count",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_module", type=str, required=True,
                        help="e.g. src.models.mobilenetv3_plus_siamese_exact or src.models.resnet18_siamese")
    parser.add_argument("--embedding_dim", type=int, default=10)
    parser.add_argument("--pretrained", action="store_true", help="Build model with ImageNet weights (only affects architecture init).")
